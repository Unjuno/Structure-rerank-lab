...
def diagnose(results_path: Path, judgments_path: Path, k: int) -> Dict[str, Any]:
    ...
    for item in per_query:
        ...
        for score in item["mode_scores"]:
            if score["mode"] == "baseline":
                continue
            if score["avg_rel"] < baseline:
                harmful_modes[score["mode"]] = harmful_modes.get(score["mode"], 0) + 1
            elif score["avg_rel"] > baseline:
                helpful_modes[score["mode"]] = helpful_modes.get(score["mode"], 0) + 1
...
def write_md(report: Mapping[str, Any], output_path: Path) -> None:
    ...
    lines.append("")
    lines.append("## Helpful modes")
    lines.append("")
    lines.append("| mode | improved query count |")
    lines.append("|---|---:|")
    for mode, count in report["helpful_modes"].items():
        lines.append(f"| {mode} | {count} |")
    if not report["helpful_modes"]:
        lines.append("| none | 0 |")
    lines.append("")
    lines.append("## Harmful modes")
    lines.append("")
    lines.append("| mode | worsened query count |")
    lines.append("|---|---:|")
    for mode, count in report["harmful_modes"].items():
        lines.append(f"| {mode} | {count} |")
    if not report["harmful_modes"]:
        lines.append("| none | 0 |")
    lines.append("")
    lines.append("## Per-query best/worst modes")
    lines.append("")
    lines.append("| query_id | baseline AvgRel | best mode | best AvgRel | worst mode | worst AvgRel |")
    lines.append("|---|---:|---|---:|---|---:|")
    for item in report["per_query"]:
        best = item["best_mode"] or {}
        worst = item["worst_mode"] or {}
        lines.append(
            f"| {item['query_id']} | {item['baseline_avg_rel']:.6f} | {best.get('mode')} | {best.get('avg_rel', 0.0):.6f} | {worst.get('mode')} | {worst.get('avg_rel', 0.0):.6f} |"
        )
    lines.append("")
...
def run(
    ...
    report = diagnose(results_path, judgments_path, k=k)
